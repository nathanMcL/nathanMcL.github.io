...
Starting with the `base image` `FROM` I am using `alpine`. There are other types of images available, from online articles it is said that `alpine` has *less of* a `visable surface`.  
...
- Next we `RUN` the install and initial system setup.  
    ...
    - This next section I want to provide a *default* `harden` `sshd` for a lab device.  
        - No root login  
    ...
    - `COPY start.sh /start.sh` plus `chmod +x`
        - *Copies* in the runtime `start.sh` file. This starts the services and runs the traffic simulation loop.  
...
- The Windows of Communication section determines how `Chatty` the device is  
    - `REQUEST_INTERVAL_SEC` controls how often a request to communicate is sent.  
